document.addEventListener("DOMContentLoaded", () => {
  const backendUrl =
    window.backendUrl || "https://senaicommunityapp.up.railway.app";
  const defaultAvatarUrl =
    window.defaultAvatarUrl || `${backendUrl}/images/default-avatar.jpg`;
  ...
  const elements = {
    ...
    profileMobileMenuToggle: document.getElementById(
  ...
  function handleProfileFeedUpdate(payload) {
    ...
    const postId =
      payload.postagemId || (postObj ? postObj.id : null) || payload.id;
    ...
    if (payload.tipo === "CRIACAO" && postObj && profileUser) {
      if (String(postObj.autorId) === String(profileUser.id)) {
        ...
        const emptyState =
          elements.postsContainer.querySelector(".empty-state");
    ...
    const subscribeToTopics = (client) => {
      ...
      if (targetId) {
        client.subscribe(`/topic/perfil/${targetId}`, (message) => {
          ...
          if (
            currentUser &&
            String(updatedUserDTO.id) === String(currentUser.id)
          ) {
  ...
  function updateProfilePageUI(user) {
    ...
    if (bannerEl && user.urlFotoFundo) {
      bannerEl.style.backgroundImage = `url('${window.getAvatarUrl(
        user.urlFotoFundo
      )}')`;
    }
    if (elements.profileName) elements.profileName.textContent = user.nome;
    if (elements.profileTitle)
      elements.profileTitle.textContent = user.tipoUsuario || "Usuário";
    ...
    if (elements.tabProjectsCount) {
      const novoTotal =
        user.totalProjetos !== undefined
          ? user.totalProjetos
          : elements.tabProjectsCount.textContent;
  ...
  function renderMediaGrid(mediaUrls) {
    ...
    let gridClass =
      count === 2 ? "double" : count === 3 ? "triple" : "multiple";
    ...
    displayItems.forEach((url, index) => {
      ...
      mediaHtml += `<div class="post-media-item${
        isMoreItem ? " more" : ""
      }" onclick="window.openMediaViewer(${safeMediaArray}, ${index})">`;
  ...
  function populateProfileData(user) {
    ...
    const finalUserImage = userImage.startsWith("http")
      ? userImage
      : `${backendUrl}${userImage.startsWith("/") ? "" : "/"}${userImage}`;
    ...
    if (bannerEl) {
      ...
      if (bgUrl && !bgUrl.includes("default-background.jpg")) {
        bgUrl = bgUrl.startsWith("http")
          ? bgUrl
          : `${backendUrl}${bgUrl.startsWith("/") ? "" : "/"}${bgUrl}`;
      } else {
    ...
    if (elements.profileDob) {
      if (user.dataNascimento) {
        ...
        elements.profileDob.textContent = new Intl.DateTimeFormat(
          "pt-BR"
        ).format(adjustedDate);
      } else {
    ...
    newEditForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (elements.profileName)
          elements.profileName.textContent = usuarioAtualizado.nome;
        if (elements.profileBio)
          elements.profileBio.textContent =
            usuarioAtualizado.bio || "Nenhuma bio informada.";
        ...
        if (
          ...
          usuarioAtualizado.urlFotoPerfil.trim() === ""
        ) {
        ...
        if (bannerEl && usuarioAtualizado.urlFotoFundo) {
          bannerEl.style.backgroundImage = `url('${window.getAvatarUrl(
            usuarioAtualizado.urlFotoFundo
          )}')`;
        }
  ...
  window.openEditProfileModal = function () {
    ...
    if (currentUser.dataNascimento) {
      document.getElementById("edit-profile-dob").value =
    ...
    if (profilePicPreview) {
      ...
      if (
        ...
        currentUser.urlFotoPerfil.trim() === ""
      ) {
    ...
    if (
      currentUser.urlFotoFundo &&
      !currentUser.urlFotoFundo.includes("default-background.jpg")
    ) {
    ...
    try {
      const url =
        userId === currentUser.id
          ? `${backendUrl}/api/amizades/`
          : `${backendUrl}/api/amizades/usuario/${userId}`;
      const response = await axios.get(url);
      renderProfileFriends(response.data);
    } catch (error) {
      if (elements.profileFriendsList)
        elements.profileFriendsList.innerHTML =
          '<p class="empty-state">Não foi possível carregar amigos.</p>';
    }
  }
  ...
  function renderProfileFriends(friends) {
    ...
    // Atualiza contador
    if (elements.tabFriendsCount)
      elements.tabFriendsCount.textContent = friends.length;
    ...
    if (friends.length === 0) {
      ...
      elements.profileFriendsList.innerHTML =
        '<p class="empty-state">Nenhum amigo encontrado.</p>';
      return;
    }
    ...
    friends.forEach((friend) => {
      ...
      const avatarUrl = window.getAvatarUrl
        ? window.getAvatarUrl(friendAvatar)
        : friendAvatar && friendAvatar.startsWith("http")
        ? friendAvatar
  ...
  async function fetchProfileProjects(userId) {
    try {
      const response = await axios.get(
        `${backendUrl}/projetos/usuario/${userId}`
      );
  ...
  function renderProfileProjects(projects) {
    ...
    if (elements.tabProjectsCount)
      elements.tabProjectsCount.textContent = projects.length;
  ...
  async function fetchUserPosts(targetUserId) {
    ...
    try {
      const response = await axios.get(
        `${backendUrl}/postagem/usuario/${targetUserId}`
      );
      elements.postsContainer.innerHTML = "";
  ...
  function createProfilePostElement(post) {
    ...
    const autorAvatar = post.urlFotoAutor
      ...
      : defaultAvatarUrl;
    const dataFormatada = new Date(post.dataCriacao).toLocaleDateString(
      "pt-BR"
    );
    const isAuthor =
      currentUser && String(post.autorId) === String(currentUser.id);
    ...
    postElement.innerHTML = `
        ...
        <div class="post-actions">
            ...
            }" onclick="window.toggleLike(event, ${post.id}, null)"><i class="${
      post.curtidoPeloUsuario ? "fas" : "far"
    } fa-heart"></i> <span id="like-count-post-${post.id}">${
        ...
        <div class="comments-section" id="comments-section-${
          post.id
            ...
            <div class="comment-form">
                ...
                <button onclick="window.sendComment(${
                  post.id
  ...
  function configureProfileActions(isMyProfile) {
    if (elements.editProfileBtnPage) {
      elements.editProfileBtnPage.style.display = isMyProfile
        ? "inline-block"
        : "none";
    }
    if (!isMyProfile && currentUser && profileUser) {
      ...
      if (profileActions) {
        ...
        messageBtn.onclick = () => {
          window.location.href = `mensagem.html?start_chat=${profileUser.id}`;
        };
        ...
        checkFriendshipStatus().then((status) => {
          switch (status) {
            ...
            case "SOLICITACAO_ENVIADA":
              friendBtn.innerHTML = '<i class="fas fa-clock"></i> Pendente';
              friendBtn.disabled = true;
              break;
            case "SOLICITACAO_RECEBIDA":
              friendBtn.innerHTML = '<i class="fas fa-user-plus"></i> Aceitar';
              friendBtn.onclick = () =>
                (window.location.href = "amizades.html");
              break;
            default:
              friendBtn.innerHTML =
  ...
  async function checkFriendshipStatus() {
    try {
      const response = await axios.get(
        `${backendUrl}/usuarios/buscar?nome=${profileUser.nome}`
      );
  ...
  async function enviarSolicitacaoAmizade() {
    try {
      await axios.post(
        `${backendUrl}/api/amizades/solicitar/${profileUser.id}`
      );
  ...
  async function removerAmizade() {
    if (confirm("Remover amizade?")) {
      try {
        ...
        const amizade = response.data.find(
          (amigo) => amigo.idUsuario === profileUser.id
        );
        if (amizade) {
          await axios.delete(
            `${backendUrl}/api/amizades/recusar/${amizade.id}`
          );
          if (window.showNotification)
            window.showNotification("Amizade removida.", "success");
          configureProfileActions(false);
        }
      } catch (error) {
        if (window.showNotification)
  ...
  function setupEventListeners() {
    if (elements.editProfileBtnPage) {
      elements.editProfileBtnPage.addEventListener("click", () => {
        if (window.openEditProfileModal) window.openEditProfileModal();
      });
    }
  ...
  function setupCarouselModalEvents() {
    if (elements.mediaViewerModal) {
      elements.mediaViewerModal.addEventListener("click", (e) => {
        if (e.target === elements.mediaViewerModal) closeMediaViewer();
      });
    }
  }
  if (
  ...
  window.openModernProjectModal = (project) => {
    ...
    const imageUrl = project.imagemUrl
      ? project.imagemUrl.startsWith("http")
        ? project.imagemUrl
        : `${window.backendUrl}/api/arquivos/${project.imagemUrl}`
      : window.defaultProjectUrl ||
        `${window.backendUrl}/images/default-project.jpg`;
    ...
    const membersHtml =
      (project.membros || [])
        .map((membro) => {
          ...
          const avatarUrl = window.getAvatarUrl
            ? window.getAvatarUrl(rawAvatar)
            : rawAvatar && rawAvatar.startsWith("http")
            ? rawAvatar
            : `${window.backendUrl}/api/arquivos/${rawAvatar}`;

          return `
              <div class="pm-member">
                  <img src="${avatarUrl}" onerror="this.src='${
        ...
        .join("") ||
      '<span style="color:var(--text-secondary)">Sem membros visíveis</span>';
          ...
          <div class="project-modal-card">
              ...
              <div class="pm-content">
                  ...
                  <div class="pm-description">
                    ${
                      project.descricao ||
                      "Este projeto não possui uma descrição detalhada."
                    }
                  </div>
                  ...
                  <div class="pm-grid">
                      ...
                      <div class="pm-info-item">
                        <h4>Equipe</h4>
                        <span>${
                          project.totalMembros || project.membros?.length || 0
                        } Membros</span>
                      </div>
                  </div>
